...
export default function LiquidityLock({ setPage }) {
  ...
  return (
    <div className="liquidity-lock">
      <div className="card">
        <OutlinedInput2 label="Search By LP Address" value={search} setValue={setSearch} />
        {
          loading &&
          <>
            <table>
        ...
        }
        {
          ...
          <table>
            ...
            <tr>
              ...
              <td><Link to={`/locks/liquidity_lock/${searchResult?.tokenAddress}`}>View</Link></td>
            </tr>
          </table>
        }
        {
            ...
            </tr>
            {
              data?.getAllLocks?.map((item, index) => (
                <tr key={index}>
                  ...
                  <td>{item.amount}</td>
                  <td><Link to={`/locks/liquidity_lock/${item.tokenAddress}`}>View</Link></td>
                </tr>
              ))
            }
          </table>
